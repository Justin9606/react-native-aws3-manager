...
export class S3Policy {
  static generate(options: any) {
    ...
    assert(
      options.bucket,
      "Must provide `bucket` option with your AWS bucket name"
    );
    assert(
      options.contentType,
    ...
    assert(
      options.accessKey,
      "Must provide `accessKey` option with your AWSAccessKeyId"
    );
    assert(
      options.secretKey,
      "Must provide `secretKey` option with your AWSSecretKey"
    );
    ...
    return formatPolicyForRequestBody(
      base64EncodedPolicy,
      signature,
      policyParams
    );
...
const formatPolicyForRequestBody = (
...
const formatPolicyForEncoding = (policy: any) => {
  return {
...
const getEncodedPolicy = (policy: any) => {
  return Buffer.from(JSON.stringify(policy), "utf-8").toString("base64");
};

const getSignature = (base64EncodedPolicy: string, options: any) => {
...
const getSignatureKey = (options: any) => {
  const kDate = CryptoJS.HmacSHA256(
    options.yyyymmddDate,
    "AWS4" + options.secretKey
  );
